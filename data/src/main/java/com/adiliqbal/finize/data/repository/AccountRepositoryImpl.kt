...
@Inject
constructor(private val accountDao: AccountDao, private val accountService: AccountService) :
    AccountRepository {

    override fun getAccounts(search: String?) = channelFlowWithAwait {
        withScope(Dispatchers.Unconfined) {
            accountDao.getAll().map { it.map { entity -> entity.toModel() } }.collect { trySend(it) }
        }
        launchSafeApi {
