...
internal class TransactionRepositoryImpl @Inject constructor(
	...
	override fun getTransactions(filter: TransactionsFilter?): Flow<PagingData<Transaction>> {
		return Pager(config = PagingConfig(pageSize = PAGE_SIZE, prefetchDistance = 0),
			remoteMediator = TransactionsRemoteMediator(
				transactionDao, transactionService, filter, logger
			),
			pagingSourceFactory = { transactionDao.getAll() }).flow.map { data -> data.map { it.toModel() } }
	}
	...
	override fun getTemplates() = channelFlowWithAwait {
		withScope(Dispatchers.Unconfined) {
			transactionDao.getTemplates().map { it.map { entity -> entity.toModel() } }
				.collect { trySend(it) }
		}
		launchSafeApi {
			templateService.getTemplates().map { it.toEntity().copy(isTemplate = true) }.let {
				transactionDao.upsert(it)
			}
		}
	}
	...
	override suspend fun createTemplate(template: Transaction) {
		return templateService.createTemplate(template.toApi()).toEntity().let {
			transactionDao.upsert(it)
			it.toModel()
		}
	}
