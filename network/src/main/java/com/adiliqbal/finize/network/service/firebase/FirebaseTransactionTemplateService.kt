...
internal class FirebaseTransactionTemplateService @Inject constructor(
	...
	override suspend fun getTemplates(): List<BaseApiTransaction> {
		return firestore.collection(transactionTemplatesDB(preferences.userId()))
			.get().result.toObjects<ApiTransaction>()
	}
	...
	override suspend fun updateTemplate(template: ApiTransaction) = firestore.update(
